...
const LockCardBottomSheet = (props:ILockCardSheet) => {
    ...
    const [lockCardData] = useState([
        {
            id: 'ld-1',
            title: "What will be blocked?",
            description: "Payments using your physical card or virtual card details, including in-store, online, ATM, and any recurring transactions."
        },
        {
            id: 'ld-2',
            title: "What still works?",
            description: "You will not be able to perform any transactions, including any recurring payments that you have set up using your card details."
        },
        {
            id: 'ld-3',
            title: "Lost or stolen?",
            description: "Please request for a new card immediately, which will also cancel your current card."
        }
    ])
  return (
    <BottomSheet
    style={{
        ...
        isVisible={isVisible}>
            <View 
            style={{ width:'100%',height: (SCREEN_HEIGHT * (SCREEN_HEIGHT > 700 ? 70 : 80)) / 100 - 55}}
            >
      ...
      <Text style={styles.subtitleStyle}>{i18n?.t("lock_card.subtitle")}</Text>
      <FlatList
        data={lockCardData}
        contentContainerStyle={styles.listContainerStyle}
        renderItem={({item}) => (
           <SettingsItemStyle item={item} /> 
        )}
