import VirtualCard from '@banking-component/wallet-component/src/components/adb-card/card-info-component/components/virtual-card';
...
const CardManagementComponent:React.FC<CardManagementProps> = (props) => {
  ...
  return (
    ...
    <View style={styles.navContainerStyle}>
      ...
      <Text style={styles.navContainerSubTitleStyle}>{"{ADB} Savings-i"}</Text>
    </View>
    <View style={{flexDirection:'row', alignItems:'center', justifyContent:'center', marginVertical: 12}}>
      <CircularImageView style={{
        containerStyle: {
          marginHorizontal: 12
        }
      }} label="Settings"/>
     <CircularImageView onClick={onLimitPress} style={{
        containerStyle: {
          marginHorizontal: 12
        }
      }} label="Limit"/>
    ...
    <View style={{marginTop: 12, justifyContent:'center', alignItems:'center', paddingHorizontal:24}}>
      <CarouselCard
       title={isVirtualCardActive ? "Do you also want a physical card?" : 'Activate virtual card!'}
       subTitle={isVirtualCardActive ? "Enjoy hassle free payments, ATM withdrawals and pay in-store merchants. " : "Activate your virtual card and start spending now!"}
