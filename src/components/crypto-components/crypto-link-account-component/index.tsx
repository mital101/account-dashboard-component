...
import { StyleProp, TextStyle, ViewStyle,StyleSheet, SafeAreaView, View, Text,TouchableOpacity,ScrollView  } from 'react-native';
...
const cryptoDummyData = [
  {
    ...
    data:[
      ...
      },
      {
        ...
        title: 'How much risk with your investments are you willing to tolerate or accept? Remember that high rewards often come with higher risks.',
        options:[
          {
            id: 'Conservative',
            title: 'Conservative',
      ...
      },
      {
        ...
        title: 'How often will you need to convert withdraw all or parts of your investments to cash?',
        options:[
          {
            id: 'Under 6 months',
...
const CryptoLinkAccountComponent = ({ style, onNext,onPressBack }: CryptoLinkAccountComponentProps) => {
  ...
  const [investmentFinancialGoal, setInvestmentFinancialGoal] = React.useState<string>();
  const [investmentRiskAppetite, setInvestmentRiskAppetite] = React.useState<string>();
  const [investmentLiquidityNeeds, setInvestmentLiquidityNeeds] = React.useState<string>();
  const [investmentTimePeriod, setInvestmentTimePeriod] = React.useState<string>();
  const [investmentProjectedTrade, setInvestmentProjectedTrade] = React.useState<string>();
  const [investmentKnowledgeLevel, setInvestmentKnowledgeLevel] = React.useState<string>();
  const [investmentNetWorth, setInvestmentNetWorth] = React.useState<string>();
  const [hasCryptoCreateApplicationError, setHasCryptoCreateApplicationError] = React.useState<string>();
  ...
  useEffect(() => {
    ...
    if (applicationList.length > 0) {
      applicationList.filter(function (el)
        {
          if (el.productDetails.bankId === 'PDAX' &&
  ...
  const createRiskProfile =async ()=>{
    ...
    if ((cryptoApplicationDetails && cryptoApplicationDetails.applicationId )|| existingApplicationId) {
      console.log('wwwwwwwww');

      const applicationId = cryptoApplicationDetails?cryptoApplicationDetails.applicationId:existingApplicationId;
      ...
      const respone2 = await customerInvokeService.submitRiskProfile(applicationId);

      setTimeout(async() => {
        const respone3 = await customerInvokeService.getApplicationStatus(applicationId);
  ...
  const radioButtonGroup =(data,type) => {
    return data.map((item,index)=>{
      return(<TouchableOpacity key={`${item.title}-${index}`} style={styles.radioBtncontainer} onPress={() =>{

        type === 'investmentFinancialGoal'?setInvestmentFinancialGoal(item.value) :
          type === 'investmentRiskAppetite'?setInvestmentRiskAppetite(item.value) :
            type === 'investmentLiquidityNeeds'?setInvestmentLiquidityNeeds(item.value) :
              type === 'investmentTimePeriod'?setInvestmentTimePeriod(item.value) :
                type === 'investmentProjectedTrade'?setInvestmentProjectedTrade(item.value) :
                  type === 'investmentKnowledgeLevel'?setInvestmentKnowledgeLevel(item.value) :
                    type === 'investmentNetWorth'?setInvestmentNetWorth(item.value) : ''
      }}>
          ...
          <View style={styles.radioBtn}>
            {type === 'investmentFinancialGoal'&& investmentFinancialGoal === item.value && <View style={styles.selectedRadioBtn} /> }
            {type === 'investmentRiskAppetite'&& investmentRiskAppetite === item.value && <View style={styles.selectedRadioBtn} /> }
            {type === 'investmentLiquidityNeeds'&& investmentLiquidityNeeds === item.value && <View style={styles.selectedRadioBtn} /> }
            {type === 'investmentTimePeriod'&& investmentTimePeriod === item.value && <View style={styles.selectedRadioBtn} /> }
            {type === 'investmentProjectedTrade'&& investmentProjectedTrade === item.value && <View style={styles.selectedRadioBtn} /> }
            {type === 'investmentKnowledgeLevel'&& investmentKnowledgeLevel === item.value && <View style={styles.selectedRadioBtn} /> }
            {type === 'investmentNetWorth'&& investmentNetWorth === item.value && <View style={styles.selectedRadioBtn} /> }
  ...
  if (isAppCreated && !cryptoErrorCreateApplication && !hasCryptoCreateApplicationError) {
    return (
      <SuccessVerificationComponent
        onNext={() => {
          onNext();
          ...
          <View style={styles.mainContainerStyle}>
            ...
            <ScrollView>
              <View style={styles.headerTitle}>
                <Text style={styles.title}>Let's find out a suitable investment strategy for you.</Text>
              </View>
              <View style={styles.headerIcon}>
                <Text style={styles.riskHeadersubTitle}>
                  This questionnaire will help you determine your profile and match a investment strategy designed for you.
                </Text>
              </View>
              <>
                ...
                return(
                  <View key={k} style={styles.contentWrapper}>
                    <Text style={styles.riskContentTitle}>
                      {obj.header}
                    </Text>
