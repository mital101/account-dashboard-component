...
const SelectTransactionTypeModal = ({
  ...
  const [selectedTypes, setSelectedTypes] = useState<TransactionTypes[]>(initialValue || []);
  const [selectedAllTypes, setSelectedAllTypes] = useState<boolean>(initialValue?.length === 0);
  ...
  const onSelectItem = (id?: string) => {
      ...
      if(indexSelected >= 0) {
        ...
        cloneSelectedTypes.push(dataTransactionTypes[dataTransactionTypes.findIndex(t => t.id === id)]);
      }
    ...
    <BottomSheet onBackButtonPress={onClose} onBackdropPress={onClose} isVisible={isVisible}>
      <View style={styles.container}>
        ...
        <View style={styles.content}>
          ...
          {dataTransactionTypes.map((itm) => 
            <CheckBox
              key={itm.id}
              title={itm.title}
              isSelected={selectedTypes.some(i => i.id === itm.id) || selectedAllTypes}
        ...
        <View style={styles.actionsView}>
          <Button label={'Select'} onPress={onSelect} disabled={!isValidToSubmit} disableColor={'#BAB7BB'}/>
        </View>
      </View>
    </BottomSheet>
  );
